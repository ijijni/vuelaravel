...
class Common extends Model 
{
	 ...
	 * @return    [type]                           [description]
	 */
	public function delDataById($id = '', $delSon = true)
	{
		$data['status'] = 1;
		// $this->startTrans();
		try {
	 ...
	 * @return    [type]                          [description]
	 */
	public function delDatas($ids = [], $delSon = true)
	{
		$data['status'] = 1;
		if (empty($ids)) {
			$data['error'] = '删除失败';
	 ...
	 * @return    [type]                           [description]
	 */
	public function enableDatas($ids = [], $status = 1, $delSon = true)
	{
		$data['status'] = 1;
		if (empty($ids)) {
			$data['error'] = '传送数据为空';
		...
		$a=$this->whereIn('id', $ids)->update(['status'=>$status]);
			return $a;
		try {
	...
	{
		$childIds = $this->where('pid',$id)->select('id')->get()->toArray();
		if (!empty($childIds)) {
			foreach ($childIds as $v) {
				$data[] = $v['id'];
				$this->getAllChild($v['id'], $data);
			}
		}
		return $data;
	}		
