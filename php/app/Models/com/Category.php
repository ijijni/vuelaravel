...
class Category {
    ...
    public function __construct($model = '', $fields = array()) {
        if (is_string($model) && (!empty($model))) {		
            if (!$this->model = DB::table($model)){
                $data['error'] = $model . "模型不存在！";
                return $data;
         }
        }
        if (is_object($model))
    ...
    private function _findAllCat($condition, $orderby = NULL) {
        if($condition == null){
            $rawList = empty($orderby) ? $this->model->get()->toArray() : $this->model->orderBy($orderby)->get()->toArray() ;
            $this->rawList=array_map('get_object_vars', $rawList);
        }else{
            $rawList = empty($orderby) ? $this->model->where($condition)->get()->toArray() : $this->model->where($condition)->orderBy($orderby)->get()->toArray() ;
             $this->rawList=array_map('get_object_vars', $rawList);
        }
     ...
     * @return  array            返回结构信息
     */
    public function getList($condition = 1, $cid = 0, $orderby = NULL) {
